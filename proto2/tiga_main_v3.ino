// ============================================================
// TIGA — Main Application v3.0
// Wearable health tracker for elderly users
//
// What's new in v3:
//   - Deep sleep: hold BTN1 3s → sleep, any button → wake
//   - Session export: after walk, open Serial Monitor 115200
//     baud and press BTN1+BTN2 together from clock screen.
//     Full medical report prints to Serial — copy and share.
//   - Human language throughout ("Heart is steady" not "87 BPM")
//   - All test screens complete with real sensor interaction
//   - Doctor's report screen (medical terms, hidden in menu)
//   - SOS as slow calm pulse — not harsh flash
//   - Settings screen with language placeholder
//   - Fall detection: 10s countdown with cancel before alerting
//   - Daily summary screen
//   - NTP time sync via WiFi (set your credentials below)
//
...
//   Button1  GPIO21→GND (scroll / toggle)
//   Button2  GPIO16→GND (select / confirm)
//   Battery  GPIO04 (internal ADC)
//   LCD pwr  GPIO15 (must HIGH)
//   GPS TX   GPIO43 (reserved — module arriving next week)
//   GPS RX   GPIO44 (reserved — module arriving next week)
// ============================================================
...
void readSensors() {
  ...
  if (mpuOK) {
    ...
    float pitch = atan2f((float)ax, sqrtf((float)ay*ay + (float)az*az)) * 180.0f / 3.14159f;
    ...
    if (inFall && millis() - fallTime > 250) {
      if (g < 0.5f) {
        // Possible fall — start confirmation countdown
        inFall = false;
    ...
    // Balance score (lower wobble = higher score)
...
void exportSession() {
  ...
  Serial.println("╠══════════════════════════════════════════════════╣");
  Serial.println("║  DEXTERITY TESTS                                   ║");
...
void drawClockFull() {
  ...
  tft.setTextDatum(MR_DATUM);
  tft.drawString("both: export", W-8, H-8);
}
...
void drawSettings() {
  ...
  tft.setTextColor(C_MUTED);  tft.drawString("arriving next week", 120, 138);
