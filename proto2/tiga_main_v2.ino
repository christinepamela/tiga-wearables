// ============================================================
// TIGA — Main Application v2.0
// Wearable health tracker for elderly users
//
// What's new in v2:
//   - Clock face as default screen (large time + date)
//   - BTN1 toggles between clock and health view
...
void drawClockFull() {
  ...
  tft.drawString("BTN1: health", 8, H-8);
  tft.setTextDatum(MR_DATUM);
  tft.drawString("BTN2: menu", W-8, H-8);
}
...
void drawSettings() {
  ...
  tft.setTextColor(C_TEXT);   tft.drawString("GPS:", 16, 118);
  tft.setTextColor(C_MUTED);  tft.drawString("arriving next week", 120, 118);

  tft.setTextColor(C_DIM);
  tft.setTextDatum(MC_DATUM);
  tft.drawString("Full settings in companion app (v2)", W/2, 142);
