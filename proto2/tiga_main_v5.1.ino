// ============================================================
// TIGA — Main Application v5.1
// Wearable health tracker for elderly users
//
// What's new in v5.1:
//   - Fix: BTN2 short-press no longer fires when starting a hold-gesture
//          (was opening menu before reset could trigger)
//   - Fix: Session reset screen now shows for 2.5s with progress bar
//   - Fix: Low HR readings (<40 bpm) now rejected as noise, not logged
//          (eliminates false "bradycardia" flag when finger off sensor)
//   - Fix: HR emergency state only triggers after 3 consecutive bad readings
//          (was triggering on single noisy read at boot)
//   - New: accelG visible on Stability screen as "Motion" value
//   - New: Optional haptic pulse on session reset (if VIBE_MOTOR_PIN set)
//
// What was new in v5:
