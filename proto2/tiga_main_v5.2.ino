// ============================================================
// TIGA — Main Application v5.2
// Wearable health tracker for elderly users
//
// What's new in v5.2:
//   - Removed SW-420 vibration sensor (redundant — MPU6050 does this)
//   - Removed TTP223 touch sensor (will be replaced by MAX30102 signal
//     validity in v6; for now "wearing" stub returns true)
//   - Dexterity screen disabled (relied on TTP223 — re-enables in v6
//     with button-tap test that doesn't need external touch sensor)
//   - GPIO3 and GPIO13 freed for future use (MAX30102 INT, buzzer)
//
// What was new in v5.1:
//   - BTN2 short-press suppression during hold gesture
//   - Session reset screen visible for 2.5s with progress bar
//   - Low-HR noise rejection (<40 bpm)
//   - HR emergency requires 3 consecutive bad readings
//   - accelG shown on Stability screen
//
// What was new in v5:
//   - MPU6050 robustness (retry, 100kHz, auto-reconnect)
//   - I2C bus recovery
//   - Adaptive step detection
//   - Session reset via BTN2 hold
//   - >6G fall spike rejection
//   - MPU health stats in export
//
...
//   Pulse    S→1      +→3V    -→GND  (to be retired in v6 for MAX30102)
...
//   Free     GPIO3, GPIO13 (reserved for MAX30102 INT, buzzer, vibe motor)
// ============================================================
...
void readSensors() {
  ...
  // Track session peak / low HR — ONLY physiologically plausible values (v5.1)
  // Prevents "Low: 17 bpm" false flags from noise when finger off sensor.
  ...
  // Wearing detection (v5.2: stub — returns true until MAX30102 signal
  // validity check replaces it in v6)
  data.wearing = true;
  ...
  // HR alert (v5.1 — requires 3 consecutive bad readings to trigger)
  // Prevents false emergencies from single noisy reads when finger lifts off.
  ...
  if (data.heartRate > 0 && (state == STATE_CLOCK || state == STATE_HEALTH)) {
    if (data.heartRate > HR_WARN_HIGH || data.heartRate < HR_WARN_LOW) {
      ...
      consecutiveBadHR = 0;  // reset counter on any good reading
    }
  }
}
...
void resetSession() {
  // Visual confirmation (v5.1 — longer, clearer, with progress bar)
  tft.fillScreen(C_BG);
  tft.setTextDatum(MC_DATUM);
  ...
  // Progress bar fills over 2 seconds so user sees it
  int barY = H - 22;
  int barW = 200;
  int barX = (W - barW) / 2;
  ...
  for (int i = 0; i <= barW; i += 4) {
    ...
    delay(40);  // 4px * 40ms = 40 steps * 40ms = ~1.6s total
  }
  ...
  delay(600);  // brief pause after bar fills

  // Return to clock for fresh session view
  state = STATE_CLOCK;
  needsFullDraw = true;
}
...
void readButtons() {
  ...
  // Edge detection (falling edge = release)
  bool b1Falling = (!b1 && btn1Last);
  bool b2Falling = (!b2 && btn2Last);

  // Default: no press this cycle
  btn1Pressed = false;
  btn2Pressed = false;

  // ── BTN1: short press = action, long hold = sleep ──────────
  if (b1) {
    ...
    // Button released — if it was a short press (not a hold), fire now
    if (b1Falling && !btn1Held && btn1HoldStart > 0) {
      btn1Pressed = true;
    }
    btn1HoldStart = 0;
    btn1Held = false;
  ...
  // ── BTN2: short press = action, long hold = reset ──────────
  // Hold gesture only valid on clock/health screens.
  bool b2HoldScreen = (state == STATE_CLOCK || state == STATE_HEALTH);
  if (b2) {
    if (btn2HoldStart == 0) btn2HoldStart = millis();
    if (!btn2Held && b2HoldScreen &&
        millis() - btn2HoldStart >= RESET_HOLD_MS) {
      btn2Held = true;
      resetSession();
    }
  } else {
    // Released — if it was a short press (not a completed hold), fire now
    if (b2Falling && !btn2Held && btn2HoldStart > 0) {
      btn2Pressed = true;
    }
    btn2HoldStart = 0;
    btn2Held = false;
...
void handleInput() {
  ...
  switch (state) {
    ...
    case STATE_DEXTERITY:
      // v5.2: placeholder stub — any button returns to menu
      if (btn1Pressed || btn2Pressed) {
        state = STATE_MENU;
        needsFullDraw = true;
      }
...
void drawStability() {
  ...
  // Fall detector status + live Motion value (v5.1)
  tft.setTextColor(C_DIM);
  char motionStr[32];
  sprintf(motionStr, "Fall detection: active  Motion: %.2fG", data.accelG);
...
// ── DEXTERITY ────────────────────────────────────────────────
void drawDexterity() {
  // Dexterity tests disabled in v5.2 — relied on TTP223 touch sensor
  // which has been removed. Will return in v6 with a button-tap based
  // test (finger tap speed + reaction time using BTN1/BTN2) plus an
  // MPU6050-based tremor test.
  drawTopBar("DEXTERITY TEST");
  tft.setTextDatum(MC_DATUM);
  tft.setTextSize(1);
  tft.setTextColor(C_MUTED);
  tft.drawString("Dexterity tests temporarily", W/2, 60);
  tft.drawString("unavailable. Coming back in v6", W/2, 78);
  tft.drawString("with improved reaction tests.", W/2, 96);
  drawBottomHint("any button: back");
}
