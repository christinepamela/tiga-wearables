// ============================================================
// TIGA — Main Application v7
// Wearable health tracker for elderly users
//
// What's new in v7:
//   - Piezo vibration sensor (LM393 module on GPIO1)
//       Delta-based tap detection with debounce
//       Haptic confirmation pulse on each tap
//       Tap count in settings, doctor report, session export
//   - v6a content unchanged (MAX30102, BMP280, alerts)
// ============================================================
...
#define I2C_SDA      18
#define I2C_SCL      17
#define PIEZO_PIN     1       // GPIO01, ADC1_CH0
...
// ── Piezo vibration sensor (LM393 module on A0) ──────────────
bool  piezoOK           = false;
int   piezoBaseline     = 2048;
int   piezoPeakDelta    = 0;
int   piezoLastDelta    = 0;
int   piezoLastRaw      = 0;
unsigned long piezoLastTapMs = 0;
int   piezoTapCount     = 0;
#define PIEZO_TAP_THRESHOLD   300   // delta required to count as a tap
#define PIEZO_TAP_DEBOUNCE_MS 250
#define PIEZO_BASE_ALPHA      0.02f
#define PIEZO_PEAK_DECAY      0.90f
...
#define IR_FINGER_THRESHOLD  5000UL
...
// ── SpO2 rolling window ──────────────────────────────────────
#define SPO2_WIN 25
long    irWin[SPO2_WIN]  = {0};
long    redWin[SPO2_WIN] = {0};
uint8_t spo2WinIdx       = 0;
bool    spo2WinFull      = false;
...
void setup() {
  ...
  if (max30102.begin(Wire, I2C_SPEED_STANDARD)) {
    ...
    max30102.setup(120,           // LED brightness 0-255 (60 = moderate)
                   1,            // sampleAverage: average 4 samples
                   2,            // ledMode: 2 = red + IR
  ...
  {
    long sum = 0;
    for (int i = 0; i < 20; i++) { sum += analogRead(PIEZO_PIN); delay(5); }
    piezoBaseline = sum / 20;
  }
  piezoOK = (piezoBaseline > 20 && piezoBaseline < 4075);
  Serial.printf("[TIGA] Piezo baseline: %d  status: %s\n",
                piezoBaseline, piezoOK ? "OK" : "check trimmer");
  ...
  Serial.println("[TIGA] v7 boot complete");
  Serial.printf("[TIGA] Sensors: MPU=%s  MAX=%s  BMP=%s  PIEZO=%s\n",
                mpuOK?"OK":"FAIL", maxOK?"OK":"FAIL",
                bmpOK?"OK":"FAIL", piezoOK?"OK":"check");
  bleSetup();
}
...
void loop() {
  ...
  if (millis() - lastUpdate >= 1000) {
    ...
    copyPrev();
    bleNotify();
    static uint8_t saveTick = 0;
    if (++saveTick >= 60) { saveTick = 0; persistSave(); }
  }
...
void readMAX30102() {
  ...
  irWin[spo2WinIdx]  = irValue;
  redWin[spo2WinIdx] = redValue;
  spo2WinIdx++;
  if (spo2WinIdx >= SPO2_WIN) { spo2WinIdx = 0; spo2WinFull = true; }
...
// ============================================================
// PIEZO VIBRATION SENSOR (LM393 module)
// Delta-based detection — same algorithm as the standalone test.
// A "tap" is counted when delta exceeds PIEZO_TAP_THRESHOLD and
...
void readPiezo() {
  ...
  // Slow-moving baseline tracks the DC offset
  piezoBaseline = (int)((1.0f - PIEZO_BASE_ALPHA) * piezoBaseline
                        + PIEZO_BASE_ALPHA * raw);

  int delta = raw - piezoBaseline;
  if (delta < 0) delta = -delta;
  piezoLastDelta = delta;

  // Peak decay so a spike stands out for a moment
  piezoPeakDelta = (int)(piezoPeakDelta * PIEZO_PEAK_DECAY);
  if (delta > piezoPeakDelta) piezoPeakDelta = delta;

  // Tap event detection
  if (delta > PIEZO_TAP_THRESHOLD &&
      millis() - piezoLastTapMs > PIEZO_TAP_DEBOUNCE_MS) {
    piezoLastTapMs = millis();
    piezoTapCount++;
    Serial.printf("[PIEZO] tap #%d  delta=%d  raw=%d  base=%d\n",
                  piezoTapCount, delta, raw, piezoBaseline);
    // Gentle feedback so the user knows it registered
    motorGentlePulse();
  }
}
...
void exportSession() {
  ...
  Serial.println();
  Serial.println("=================================================");
  Serial.println("  TIGA WALK TEST REPORT — v7");
  Serial.println("=================================================");
  ...
  Serial.println();
  Serial.println("  [7] VIBRATION (PIEZO)");
  Serial.printf ("  Taps:      %d\n", piezoTapCount);
  Serial.printf ("  Baseline:  %d\n", piezoBaseline);
  Serial.printf ("  Last raw:  %d\n", piezoLastRaw);
  Serial.printf ("  Last delta:%d\n", piezoLastDelta);
  Serial.printf ("  Status:    %s\n", piezoOK ? "OK" : "check trimmer");
...
void drawDoctor() {
  ...
  sprintf(s, "%d taps", piezoTapCount);   drow("Piezo taps:", s);
  drawBottomHint("any button: back");
}
...
void drawSettings() {
  ...
  sprintf(s, "raw=%d d=%d taps=%d", piezoLastRaw, piezoLastDelta, piezoTapCount);
  srow("Piezo:", s, piezoOK ? C_GREEN : C_RED);
  if (gpsData.hasFix) {
